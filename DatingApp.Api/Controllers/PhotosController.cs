...
namespace DatingApp.Api.Controllers
{
    ...
    public class PhotosController : ControllerBase
    {
        ...
        public PhotosController(IDatingRepository repo,
            IMapper mapper, IOptions<CloudinarySettings> cloudinaryConfig, Microsoft.Extensions.Configuration.IConfiguration config)
        {
            _repo = repo;
            _mapper = mapper;
        ...
        private string UploadPhotoToBlob(IFormFile pFile,int userId)
        {
            using (var ms = new MemoryStream())
            {
                pFile.CopyTo(ms);
                var fileName = Path.GetFileName(pFile.FileName);
                string mimeType = pFile.ContentType;
                byte[] fileData = ms.ToArray();
                BlobStorageService objBlobService = new BlobStorageService(_config);
             return objBlobService.UploadFileToBlob(fileName, fileData, mimeType, userId);
            }
        }
        [HttpPost("{id}/setMain")]
        public async Task<IActionResult> SetMainPhoto(int userId, int id)
        {
            ...
            if (!user.Photos.Any(pX => pX.Id == id))
            {
                return Unauthorized();
            }
            ...
            var currentMainPhoto = await _repo.GetMainPhotoForUserPhoto(userId);
        ...
        public async Task<IActionResult> DeletePhoto(int userId, int id)
        {
            ...
            return BadRequest("Failed dot delete the photo");
        }
    }
}